@extends('layout.app')
@section('content')
    <a href="/worker/task-list/{{ $task_detail->task->id }}" class="btn btn-danger mb-3 mt-5"
        onclick="return confirm('kembali akan menghapus semua progress!')">kembali</a>
    <form action="/worker/task-list/{{ $task_detail->id }}/post-edit" method="post" autocomplete="off">
        @csrf
